...
class TPManager(object):
	...
	def init(self, vFile):
		self.bios.new(vFile)
	def destroy(self, vFile):
		self.path.fileName = 'test.tp'
		self.bios.delete(path)
	def new(self, path):
		self.load()
		if self.exist(path):
			pass
	...
	# Private function
	def load(self):
		self.bios.getVFile(self.path.path)
		self.TPFileAnalyze()
		return self.data
	def save(self):
		newData, newContent = [], []
		...
		for file in self.data:
			if isExist(file):
				print('[INFO][TPOS-save]Found a same file in TP file.')
			else:
				newData.append(file)
		for file in newData:
			fileHead = True
...
if __name__ == '__main__':
	I = TPManager()
# TEST function-init(path, content = []):void
